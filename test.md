...
| **test-active-user2** | 作成48日前<br/>未使用 | 作成57分前<br/>未使用 | **COMPLIANT**<br/>（キー作成57分前 < 30日）| **COMPLIANT**<br/>（パス作成48日前 < 90日）|
| **test-active-user3** | 作成47分前<br/>未使用 | なし | **COMPLIANT**<br/>（パス作成47分前 < 30日）| **COMPLIANT**<br/>（パス作成47分前 < 90日）|

## 判定ロジック（OR条件）
以下のいずれかが設定期間内であればCOMPLIANT：
1. **パスワード最終使用日** < 設定期間
2. **パスワード作成日** < 設定期間（未使用の場合）
3. **アクセスキー最終使用日** < 設定期間
4. **アクセスキー作成日** < 設定期間（未使用の場合）
...
- `test-active-user2`: **COMPLIANT**（アクセスキー作成57分前 < 30日）
